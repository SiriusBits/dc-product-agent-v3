...
def _build_extraction_metadata(
    *,
    ...
    if extraction_date:
        ...
        metadata["extraction_date"] = _dt.datetime.now(_dt.timezone.utc).isoformat(
            timespec="seconds"
        )
...
def convert_base_yaml(
    ...
    *,
    ...
    document_type = (
        expanded.get("document_type") or defaults.get("document_type") or "Technical Bulletin"
    )
    ...
    extraction_metadata = _build_extraction_metadata(
        extractor_version=extraction_metadata_input.get("extractor_version")
    ...
    output["properties_and_specifications"] = _ensure_list(
        expanded.get("properties_and_specifications")
    )
    ...
    for item in images_input:
        if isinstance(item, dict):
            ...
            if (
                "image_path" not in image_record
                and processed_images_root
                and image_record.get("filename")
            ):
                image_record["image_path"] = (
                    f"{processed_images_root.rstrip('/')}/{pdf_stem}/{image_record['filename']}"
                )
