...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if not schema_dir or not schema_dir.exists():
        print(
            "Schema directory not found. Provide --schema-dir or use --no-validate.",
            file=sys.stderr,
        )
        return 2
    ...
    if failures:
        print(
            f"Converted {len(results)} file(s) with {failures} validation failure(s)",
            file=sys.stderr,
        )
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pdf-data-extractor", description="PDF metadata and schema utilities"
    )
    ...
    sync_parser = subparsers.add_parser(
        "sync-metadata", help="Update base JSON/YAML with metadata from PDFs"
    )
    sync_parser.add_argument(
        "--json-dir", required=True, help="Directory containing *_base.json files"
    )
    sync_parser.add_argument(
        "--pdf-root",
        help="Directory containing source PDFs (uses source_filepath fallback when omitted)",
    )
    sync_parser.add_argument(
        "--yaml-dir", help="Directory containing *_base.yaml files to keep in sync"
    )
    sync_parser.add_argument(
    ...
    validate_parser.add_argument(
        "--schema-type", default="base", choices=sorted(SCHEMA_FILES.keys())
    )
    validate_parser.set_defaults(func=cmd_validate)

    ingest_parser = subparsers.add_parser(
        "ingest", help="Convert base YAML extracts into JSON and validate"
    )
    ingest_parser.add_argument(
        "--base-yaml-dir", required=True, help="Directory containing *_base.yaml files"
    )
    ingest_parser.add_argument(
        "--output-dir", required=True, help="Destination directory for generated JSON"
    )
    ingest_parser.add_argument("--pdf-root", help="Directory containing source PDFs")
    ingest_parser.add_argument(
        "--constants", help="Path to constants YAML for placeholder expansion"
    )
    ingest_parser.add_argument(
        "--extractor-version", help="Override extractor version for extraction metadata"
    )
    ingest_parser.add_argument(
        "--extraction-date",
        help="Optional ISO timestamp to use for extraction_metadata.extraction_date",
    )
    ingest_parser.add_argument("--schema-dir", help="Schema directory used for validation")
    ingest_parser.add_argument(
        "--no-validate", action="store_true", help="Skip schema validation after conversion"
    )
