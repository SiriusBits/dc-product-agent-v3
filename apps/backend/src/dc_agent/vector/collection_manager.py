...
class CollectionManager:
    ...
    async def initialize_default_collections(self) -> dict[str, bool]:
        ...
        for collection_type, config in self.default_collections.items():
            ...
            success = await self.vector_store.create_collection(
                name=collection_name, dimension=config["dimension"], metadata=metadata
            )
    ...
    async def create_custom_collection(
        ...
        success = await self.vector_store.create_collection(
            name=name, dimension=dimension, metadata=collection_metadata
        )
    ...
    async def backup_collection(self, name: str) -> dict[str, Any] | None:
            ...
            for collection in collections:
                ...
                info = {
                    **collection,
                    ...
                    "is_default": collection["name"]
                    in [ct.value for ct in CollectionType],
    ...
    async def validate_collection_health(self, name: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"collection_name": name, "status": "error", "errors": [str(e)]}
    ...
    async def get_collection_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            default_collections = sum(
                1 for col in collections if col.get("is_default", False)
            )
            ...
            unhealthy_count = sum(
                1 for h in health_checks if h["status"] in ["unhealthy", "error"]
            )
