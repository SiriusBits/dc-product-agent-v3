...
class VectorStoreConfig:
    ...
    @classmethod
    def from_env(
        cls, store_type: VectorStoreType = VectorStoreType.CHROMA
    ) -> "VectorStoreConfig":
    ...
    def from_env(cls, model_name: str = "nomic-embed-text") -> "EmbeddingConfig":
        ...
        if model_name in default_configs:
            ...
            config.batch_size = int(
                os.getenv("EMBEDDING_BATCH_SIZE", str(config.batch_size))
            )
            config.max_length = int(
                os.getenv("EMBEDDING_MAX_LENGTH", str(config.max_length))
            )
