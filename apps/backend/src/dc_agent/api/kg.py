...
@router.get("/relationships/{relationship_type}", response_model=ApiResponse[List[dict]])
async def get_relationships_by_type(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to query relationships: {str(e)}"
        )
...
async def get_kg_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get KG statistics: {str(e)}"
        )
...
async def validate_kg_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to validate knowledge graph: {str(e)}"
        )