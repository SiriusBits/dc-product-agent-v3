...
from ..models.api_models import RetrievalResult, QueryAnalysis
...
class HybridRetrievalService:
    ...
    async def search(
        ...
        kg_weight: Optional[float] = None,
        collections: Optional[List[str]] = None,
        kg_relationship_types: Optional[List[str]] = None,
        min_score: float = 0.1
        ...
        try:
            ...
            if collections is None:
                collections = query_analysis.suggested_strategy.get("collections", ["technical_bulletins"])
            
            # Use provided relationship types or fall back to analyzed strategy
            if kg_relationship_types is None:
                kg_relationship_types = query_analysis.suggested_strategy.get("kg_relationship_types")
            
            # Determine number of results to fetch from each source
            vector_k = max(1, int(max_results * vector_weight * 1.5))  # Fetch extra for fusion
            kg_k = max(1, int(max_results * kg_weight * 1.5))
            
            logger.info(f"Hybrid search: vector_weight={vector_weight:.2f}, kg_weight={kg_weight:.2f}")
            ...
            error_analysis = QueryAnalysis(
                query_type=query_analysis.query_type if 'query_analysis' in locals() else "general",
                entities=[],
        ...
        try:
            ...
            for result in vector_results:
                weighted_result = RetrievalResult(
                    ...
                    metadata={**result.metadata, "original_score": result.score, "weight_applied": vector_weight},
            ...
            for result in kg_results:
                weighted_result = RetrievalResult(
                    ...
                    source=result.source,
                    metadata={**result.metadata, "original_score": result.score, "weight_applied": kg_weight},
                    provenance=result.provenance
                )
    ...
    def _remove_duplicate_results(self, results: List[RetrievalResult]) -> List[RetrievalResult]:
        ...
        for result in results:
            ...
            for existing in deduplicated:
                if self._calculate_content_similarity(result.content, existing.content) > 0.9:
    ...
    def _normalize_content_for_comparison(self, content: str) -> str:
        ...
        prefixes_to_remove = [
            "product:", "entity:", "section:", "properties:", "applications:"
        ]
    ...
    def _apply_diversity_boost(
        ...
        if len(source_groups) > 1:
            ...
            remaining_results = []
            for source, source_results in source_groups.items():
                remaining_results.extend(source_results[1:])  # Skip first (already added)
    ...
    async def get_related_content(
        ...
        try:
            ...
            if kg_response.central_entity and kg_response.central_entity.id != "not_found":
            ...
            for entity in kg_response.related_entities:
                ...
                if entity.metadata:
                    content += "Properties:\n"
                    for key, value in list(entity.metadata.items())[:3]:  # Limit properties
                        content += f"  {key}: {value}\n"
