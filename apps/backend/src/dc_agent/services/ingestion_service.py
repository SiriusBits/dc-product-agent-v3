...
class IngestionService:
    ...
    async def ingest_extraction_document(
        ...
        try:
            ...
            if document.doc_id and not overwrite:
                ...
                if existing_chunks:
                    logger.warning(f"Document {document.doc_id} already exists in {collection_name}")
                    return {
                        "success": False,
            ...
            if document.doc_id and overwrite:
                await self.vector_service.delete_document(document.doc_id, collection_name)
                logger.info(f"Deleted existing document {document.doc_id} for overwrite")
            ...
            logger.error(f"Failed to ingest extraction document {document.filename}: {e}")
            return {
                "success": False,
                "error": str(e),
                "document_id": document.doc_id,
    ...
    async def batch_ingest_documents(
        ...
        try:
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        if not continue_on_error:
                            logger.error(f"Stopping batch ingestion due to error: {result.get('error')}")
                            break
            ...
            logger.info(f"Batch ingestion completed: {successful}/{len(documents)} successful")
            return batch_result
    ...
    async def get_document_info(
        ...
        """
        try:
            chunks = await self.vector_service.get_document_chunks(document_id, collection_name)
    ...
    async def delete_document(
        ...
        try:
            ...
            # Delete the document
            success = await self.vector_service.delete_document(document_id, collection_name)
            ...
            if success:
                logger.info(f"Deleted document {document_id} with {doc_info['total_chunks']} chunks")
            else:
    ...
    async def get_ingestion_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for collection in collections:
                stats = await self.vector_service.get_collection_stats(collection["name"])
