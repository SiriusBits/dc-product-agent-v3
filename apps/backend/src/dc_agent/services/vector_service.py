...
from ..vector import CollectionManager, DocumentChunk, VectorStore
...
class VectorService:
    ...
    def __init__(self, vector_store: VectorStore | None = None):
        ...
        if vector_store is None:
            self.vector_store = VectorStoreFactory.create_from_env(
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if success_count == total_count:
                logger.info(
                    f"Successfully initialized {success_count}/{total_count} collections"
                )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            collections_healthy = (
                collection_summary.get("health_summary", {}).get("unhealthy", 0) == 0
            )

            overall_status = (
                "healthy" if store_healthy and collections_healthy else "degraded"
            )
    ...
    async def ingest_document(
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully ingested document {document.filename} with {len(chunks)} chunks"
                )
            else:
    ...
    async def search(
        ...
        try:
            ...
            logger.info(
                f"Found {len(retrieval_results)} results for query in {collection_name}"
            )
    ...
    async def search_multiple_collections(
        ...
        try:
            ...
            logger.info(
                f"Found {len(all_results)} total results across {len(collection_names)} collections"
            )
            return all_results
    ...
    async def delete_document(
        ...
        try:
            ...
            # Delete chunks
            success = await self.vector_store.delete_documents(
    ...
    async def _create_document_chunks(
        self, document: BaseExtractionDocument, chunk_strategy: str = "semantic"
    ) -> list[DocumentChunk]:
        ...
        try:
            ...
            if document.applications:
                applications_content = (
                    f"Applications for {document.product_info.product_name}:\n"
                )
                applications_content += "\n".join(
                    f"• {app}" for app in document.applications
                )
            ...
            if document.properties_and_specifications:
                properties_content = self._format_properties(
                    document.properties_and_specifications
                )
            ...
            if document.typical_properties and document.typical_properties.data:
                typical_props_content = self._format_typical_properties(
                    document.typical_properties
                )
            ...
            logger.info(
                f"Created {len(chunks)} chunks for document {document.filename}"
            )
            return chunks

        except Exception as e:
            logger.error(
                f"Failed to create chunks for document {document.filename}: {e}"
            )
            return []
