...
from ..models.kg_models import KGEntity, KGTriple, KnowledgeGraph, KGProvenance
...
class Neo4jKnowledgeGraphStore(KnowledgeGraphStore):
    ...
    async def find_entities(
        ...
        try:
            ...
            if text:
                conditions.append("(e.text CONTAINS $text OR any(alias IN e.aliases WHERE alias CONTAINS $text))")
                params["text"] = text
    ...
    async def create_relationship(self, triple: KGTriple) -> bool:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                if result.single():
                    logger.debug(f"Created relationship: {subject_id} -[{triple.predicate}]-> {object_id}")
                    return True
                else:
                    logger.warning(f"Failed to create relationship: entities not found")
                    return False
    ...
    async def delete_relationship(
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                if deleted_count > 0:
                    logger.debug(f"Deleted relationship: {subject_id} -[{predicate}]-> {object_id}")
                    return True
                else:
                    logger.warning(f"Relationship not found for deletion")
                    return False
    ...
    async def traverse_graph(
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                related_entities = [
                    self._node_to_entity(node) 
                    for node in record["related_entities"]
                ]
                ...
                for path in record["paths"]:
                    for rel in path.relationships:
                        ...
                        triple = self._relationship_to_triple(rel, path.start_node, path.end_node)
                        if triple:
    ...
    async def batch_create_entities(self, entities: List[KGEntity]) -> Dict[str, bool]:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                logger.info(f"Batch created {sum(results.values())}/{len(entities)} entities")
                return results
    ...
    async def batch_create_relationships(self, triples: List[KGTriple]) -> Dict[str, bool]:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                logger.info(f"Batch created {sum(results.values())}/{len(triples)} relationships")
                return results
    ...
    async def ingest_knowledge_graph(self, kg: KnowledgeGraph) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Ingested knowledge graph: {successful_entities} entities, {successful_relationships} relationships")
            return result
            ...
            return {
                "success": False,
                "error": str(e),
                "entities": {"total": len(kg.entities), "successful": 0, "failed": len(kg.entities)},
                "relationships": {"total": len(kg.kg_triples), "successful": 0, "failed": len(kg.kg_triples)}
            }
    ...
    async def export_subgraph(
        ...
        try:
            ...
            return KnowledgeGraph(entities=[], kg_triples=[], metadata={"error": str(e)})
    ...
    async def get_statistics(self) -> GraphStatistics:
        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                # Get basic counts
                entity_count_result = session.run("MATCH (n:Entity) RETURN count(n) as count")
                total_entities = entity_count_result.single()["count"]
                
                rel_count_result = session.run("MATCH ()-[r:RELATIONSHIP]->() RETURN count(r) as count")
                ...
                """)
                relationship_types = {record["predicate"]: record["count"] for record in rel_types_result}
                
                # Calculate average degree
                avg_degree = (2 * total_relationships) / total_entities if total_entities > 0 else 0.0
    ...
    def _node_to_entity(self, node) -> Optional[KGEntity]:
    ...
    def _record_to_triple(self, record) -> Optional[KGTriple]:
    ...
    def _relationship_to_triple(self, rel, start_node, end_node) -> Optional[KGTriple]:
