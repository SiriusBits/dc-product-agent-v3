...
from ..models.kg_models import DerivedInfo, KGEntity, KGTriple
...
class KnowledgeGraphService:
    ...
    async def ingest_derived_info(self, derived_info: DerivedInfo) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self.kg_store.ingest_knowledge_graph(
                derived_info.knowledge_graph
            )
            ...
            if result["success"]:
                logger.info(
                    f"Successfully ingested knowledge graph for document {derived_info.doc_id}"
                )
            else:
                logger.error(
                    f"Failed to ingest knowledge graph for document {derived_info.doc_id}"
                )
            ...
            logger.error(
                f"Failed to ingest derived info for {derived_info.doc_id}: {e}"
            )
            return {
                "success": False,
                "error": str(e),
    ...
    async def search_entities(
        self, query: str, entity_types: list[str] | None = None, limit: int = 20
        ...
        try:
            ...
            if not entities:
                ...
                entities = await self.kg_store.find_entities(
                    canonical_name=entity_name, limit=5
                )
            ...
            logger.info(
                f"Found {len(traversal_result.related_entities)} related entities for {entity_name}"
            )
            return response
    ...
    async def find_related_products(
        ...
        try:
            ...
            entities = await self.kg_store.find_entities(
                text=product_name, entity_type="PRODUCT", limit=5
            )
            ...
            for entity in traversal_result.related_entities:
                if entity.type == "PRODUCT" and entity.id != product_entity.id:
                    ...
                    relationships = [
                        r
                        for r in traversal_result.relationships
                        if (
                            isinstance(r.subject, KGEntity)
                            and r.subject.id == entity.id
                        )
                        or (isinstance(r.object, KGEntity) and r.object.id == entity.id)
                    ]
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "total_entities": 0, "total_relationships": 0}
    ...
    async def validate_graph_integrity(self) -> dict[str, Any]:
        ...
        try:
            ...
            orphaned_count = (
                orphaned_result[0]["orphaned_count"] if orphaned_result else 0
            )

            if orphaned_count > 0:
                warnings.append(
                    f"Found {orphaned_count} orphaned entities with no relationships"
                )
            ...
            no_canonical_result = await self.kg_store.execute_cypher_query(
                no_canonical_query
            )
            no_canonical_count = (
                no_canonical_result[0]["no_canonical_count"]
                if no_canonical_result
                else 0
            )

            if no_canonical_count > 0:
                warnings.append(
                    f"Found {no_canonical_count} entities without canonical names"
                )
            ...
            if duplicate_result:
                ...
                if duplicate_groups > 0:
                    warnings.append(
                        f"Found {duplicate_groups} groups with {duplicate_entities} potentially duplicate entities"
                    )
