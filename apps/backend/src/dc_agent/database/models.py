...
class Message(Base):
    ...
    conversation_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("metadata.conversations.id", ondelete="CASCADE")
    )
...
class KnowledgeGraphEntity(Base):
    ...
    source_document_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("metadata.documents.id")
    )
...
class QueryLog(Base):
    ...
    sources_used: Mapped[list[str] | None] = mapped_column(ARRAY(String))
    satisfaction_score: Mapped[int | None] = mapped_column(Integer)  # 1-5 rating
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
